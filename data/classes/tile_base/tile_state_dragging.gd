...
func _handle_input() -> void:
	...
	if Input.is_action_just_pressed("mouse_left"):
		...
		if actor.data:
			# 1. 检查是否需要挨着路
			if actor.data.requires_road:
				can_place_logically = can_place_logically and ConnectivityManager.check_placement_connectivity(target_grid, is_hq)
			
			# 2. 【新增】检查是否必须挨着 HQ 连通网！
			if actor.data.get("requires_hq_adjacency") and not is_hq:
				can_place_logically = can_place_logically and ConnectivityManager.has_hq_network_neighbor(target_grid)
		# 如果可以放置，计算距离和钱
